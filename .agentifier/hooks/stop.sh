...
# Auto-commit after agent session ends (when AGENTIFIER_AUTO_COMMIT_ON_STOP=1)
if [ "${AGENTIFIER_AUTO_COMMIT_ON_STOP:-0}" = "1" ] && [ -d "$PROJECT_ROOT/.git" ]; then
  cd "$PROJECT_ROOT"
  if [ -n "$(git status --porcelain)" ]; then
    MSG="${AGENTIFIER_COMMIT_MESSAGE:-chore: agent session changes}"
    git add -A
